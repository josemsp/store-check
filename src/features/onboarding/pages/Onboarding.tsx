...
const Onboarding = () => {
    ...
    // Store
    const { profileData, setProfileData, companyData, setCompanyData } = useOnboardingStore();
    ...
    const { mutate, data: validationData, isPending: isLoading, isError } = useValidateToken();
    ...
    useEffect(() => {
        if (token) {
            mutate({ token }, {
                ...
                onError: () => setStep('validating') // Will trigger error view due to isError
            });
        }
    }, [token, mutate]);

    const handleProfileComplete = async (pData: { firstName: string; lastName: string; password: string; avatarFile?: File }) => {
                ...
                <Card className="w-full max-w-md border-red-200 shadow-lg">
                    ...
                    <CardContent>
                        <p className="text-slate-600 dark:text-slate-300">
                            ...
                            Por favor, asegúrate de haber copiado el enlace completo.
                        </p>
                    </CardContent>
                </Card>
            </div>
            ...
            <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-4">
                <Card className="w-full max-w-md shadow-lg">
                    <CardHeader>
                        ...
                        <CardDescription className="text-center">Por favor espere un momento...</CardDescription>
                    </CardHeader>
                ...
                <Card className="w-full max-w-md border-red-200 shadow-lg">
                    ...
                    <CardContent>
                        <p className="text-sm text-slate-600 dark:text-slate-300">
                            Esto puede deberse a que el enlace ha expirado, ya ha sido utilizado, o no es correcto.
                        </p>
                    </CardContent>
                    <CardFooter>
            ...
            <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-4">
                <Card className="w-full max-w-md shadow-xl animate-in fade-in zoom-in duration-300">
                    ...
                    <CardContent className="space-y-4">
                        <p className="text-sm text-slate-600 dark:text-slate-300">
                            Hola <strong className="text-slate-900 dark:text-slate-100">{validationData.email}</strong>.
                            Estás a un paso de configurar tu cuenta como dueño {validationData.isNewCompany ? ' de tu nueva empresa' : ''}.
                        </p>
                        ...
                        <div className="p-6 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-800/50 space-y-4">
                            <div className="text-center space-y-2">
                                <p className="font-medium text-slate-900 dark:text-slate-100">Configurar Cuenta</p>
                                <p className="text-xs text-muted-foreground">
                                    Completa tu perfil {validationData.isNewCompany ? 'y configura tu espacio de trabajo' : ''}.
                                </p>
                            </div>
            ...
            <Card className="w-full max-w-lg shadow-xl animate-in fade-in slide-in-from-bottom duration-300">
                ...
                <div className="w-full h-1 bg-slate-100 dark:bg-slate-800">
                    <div
                        ...
                        style={{ width: step === 'profile' ? (validationData?.isNewCompany ? '50%' : '100%') : '100%' }}
                    />
                </div>
                ...
                <CardContent className="pt-6">
                    {step === 'profile' && (
                        <OnboardingProfileForm
                            ...
                            initialEmail={validationData?.email}
